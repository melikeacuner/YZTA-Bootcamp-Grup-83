...
| **ultra** | Maximum compression. Telegram-style. Only essential words survive |

## Commit Messages (caveman-commit)

Conventional Commit format. Subject ≤50 chars. Why over what. Body optional, max 3 bullet points.

```
fix(auth): token expiry off-by-one

- Was `<` need `<=` for boundary
- Add test cover edge case
```

## Code Review (caveman-review)

One-line PR comments. Format: `L{line}: {emoji} {type}: {issue}. {fix}.`

Emoji key: 🔴 bug, 🟡 smell, 🟢 nit, 💡 idea
