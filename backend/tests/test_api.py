...
def test_record_chat_and_close_flow(client):
    test_client, qdrant_repo = client
    token = _register_and_login(test_client, "chatflow@example.com")
    headers = {"Authorization": f"Bearer {token}"}
    ...
    test_client.post(f"/api/v1/sessions/{session_id}/complete", headers=headers)
    ...
    assert record_resp.json()["data"]["resolution_status"] == "open"

    # Chat with the record
    chat_resp = test_client.post(
        f"/api/v1/records/{record_id}/chat",
        json={"message": "Kök neden için aksiyon planımız nedir?"},
        headers=headers,
    )
    assert chat_resp.status_code == 200
    chat_data = chat_resp.json()["data"]
    assert "resolution_chat_history" in chat_data["meta_data"]
    assert len(chat_data["meta_data"]["resolution_chat_history"]) == 3

    # Close the record
    close_resp = test_client.post(
        f"/api/v1/records/{record_id}/close",
        headers=headers,
    )
    assert close_resp.status_code == 200
    closed_data = close_resp.json()["data"]
    assert closed_data["resolution_status"] == "closed"
