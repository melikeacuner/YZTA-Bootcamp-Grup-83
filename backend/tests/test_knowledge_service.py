...
@pytest.mark.asyncio
async def test_document_upload_and_deletion(ctx):
    session, owner_id, problem_session = ctx
    service, _ = _make_service(session)

    record = await service.create_from_session(
        problem_session, owner_id, title="Test Record", lessons_learned=VALID_LESSONS_LEARNED
    )
    
    # Upload document test
    updated_rec = await service.upload_and_summarize_document(
        record.id, "test_file.txt", b"Test teknik dokuman icerigi", "text/plain"
    )
    docs = updated_rec.meta_data.get("documents", [])
    assert len(docs) == 1
    assert docs[0]["filename"] == "test_file.txt"

    # Delete document test
    doc_id = docs[0]["id"]
    after_del = await service.delete_document(record.id, doc_id)
    assert len(after_del.meta_data.get("documents", [])) == 0


@pytest.mark.asyncio
async def test_generate_a3_preview(ctx):
    session, owner_id, problem_session = ctx
    service, _ = _make_service(session)

    record = await service.create_from_session(
        problem_session, owner_id, title="A3 Test Record", lessons_learned=VALID_LESSONS_LEARNED
    )
    a3_preview = await service.generate_a3_preview(record.id)
    assert "title" in a3_preview
    assert "root_cause" in a3_preview
    assert "corrective_actions" in a3_preview
