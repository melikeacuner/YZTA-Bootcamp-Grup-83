...
from app.infrastructure.db.models import User, ProblemSession
...
async def test_five_why_completes_after_minimum_three_steps(db_session):
    ...
    answers = [
        "Motor blogundaki yag sizintisi ve basincli conta kapaginin asinmasi nedeniyle hararet yapildi",
        "Vida gevsemesi ve baglanti somunlarinin tork degerinin dusuk kalmasi contayi zorladi",
        "Periyodik bakim çizelgesinin aksatilmasi ve vardiya degisimindeki denetim eksikligi buna yol aciti"
    ]
    for ans in answers:
        await service.submit_step_response(problem_session, ans)
    await session.commit()
    problem_session = await session.get(ProblemSession, problem_session.id)
