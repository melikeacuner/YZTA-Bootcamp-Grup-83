...
class SessionResponse(BaseModel):
    ...
    circular_logic_warning: Optional[bool] = None
    category_suggestion: Optional[dict] = None
    ...
    def from_session(cls, session: ProblemSession) -> "SessionResponse":
        ...
        return cls(
            ...
            answers=session.step_responses or step_data.get("answers", {}),
            ...
            next_prompt=step_data.get("next_prompt"),
            department=session.department,
            summary=session.summary,
            tags=session.tags or [],
            circular_logic_warning=step_data.get("circular_logic_warning"),
            category_suggestion=step_data.get("category_suggestion"),
            similar_problems=step_data.get("similar_problems")
        )
...
class SendToPoolRequest(BaseModel):
...
async def agent_chat(
    ...
    session_obj = await _get_owned_session(db, session_id, current_user)
    if session_obj.status not in ["active", "pool"]:
        raise HTTPException(status_code=400, detail="Bu oturum aktif olmadigi icin sohbet edilemez.")

    # Handle chat history rewind if user edited an earlier message
    if payload.rewind_index is not None and isinstance(payload.rewind_index, int):
        history = list(session_obj.agent_chat_history or [])
        if 0 <= payload.rewind_index < len(history):
            session_obj.agent_chat_history = history[:payload.rewind_index]
            await db.flush()
        
    mode = "resolution" if session_obj.status == "pool" else "rca"
    reply = await agent_service.chat(session_obj, payload.message, mode=mode)
    await db.commit()
    return APIResponse.ok({"reply": reply})


@router.post("/{session_id}/send-to-pool")
async def send_to_pool(
    session_id: uuid.UUID,
    payload: SendToPoolRequest,
    current_user: User = Depends(get_current_user),
    ...
    # Extract answers or chat history to derive title and root cause if missing
    answers = session_obj.step_responses or {}
    history_str = "\n".join([f"{msg['role']}: {msg['content']}" for msg in session_obj.agent_chat_history or []])
    
    root_cause_str = payload.root_cause or "Analiz sürecinde kök neden tespit edildi."
    dept_str = payload.department or session_obj.department or "Kalite"
    
    # Check if record already exists for this session
    from sqlalchemy import select
    existing_rec = (await db.execute(select(ProblemRecordORM).where(ProblemRecordORM.session_id == session_id))).scalar_one_or_none()
    
    if existing_rec:
        record = existing_rec
        record.root_cause = root_cause_str
        record.department = dept_str
        record.resolution_status = "open"
    else:
        # Create new Problem Record in pool
        record = ProblemRecordORM(
            session_id=session_obj.id,
            user_id=current_user.id,
            title=session_obj.summary or f"Vaka - {session_obj.problem_description[:35]}...",
            ...
            corrective_actions="Problem Havuzunda kalıcı eylem planlaması bekleniyor.",
            lessons_learned="Kök neden tespiti yapıldı, havuz sürecine aktarıldı.",
            department=dept_str,
            tags=payload.tags or session_obj.tags or ["kök-neden-bulundu", "pool"],
    ...
    # Automatically create default task in Action Plan under 'todo'
    new_task = Task(
        problem_record_id=record.id,
        session_id=session_obj.id,
        title=f"Kök Neden Kalıcı Giderme: {record.title}",
        description=f"Kök Neden: {root_cause_str}\nKalıcı aksiyon ve SOP planlanmalı.",
        status="todo",
        priority="medium",
        department=dept_str,
        assignee_name=session_obj.assignee_name or "Atanacak"
    )
    db.add(new_task)

    session_obj.status = "pool"
    session_obj.department = dept_str
    await db.commit()

    return APIResponse.ok({
        "message": "Kök neden başarıyla Problem Havuzuna aktarıldı ve Aksiyon Planına 'Yapılacaklar' olarak eklendi.",
...
async def delete_session(
    ...
    await db.commit()
    return APIResponse.ok({"message": "Analiz seansı başarıyla silindi ve iptal edildi."})
