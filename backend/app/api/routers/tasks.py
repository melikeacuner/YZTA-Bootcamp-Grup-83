...
async def list_tasks(
    ...
    if start_date:
        stmt = stmt.where(Task.deadline >= start_date)
    if end_date:
        stmt = stmt.where(Task.deadline <= end_date)
    ...
    mutated = False
    for t in tasks:
        # Automatic overdue transition check
        if t.deadline and t.deadline.replace(tzinfo=None) < now_utc and t.status not in ["completed", "delayed"]:
            t.status = "delayed"
            mutated = True
    ...
    return APIResponse.ok(tasks_data)
...
async def create_task(
    ...
    audit = AuditLog(
        ...
        entity_type="task",
        entity_id=task.id,
        after_state={"title": task.title, "assignee": task.assignee_name, "priority": task.priority}
    )
    db.add(audit)
    await db.commit()
