...
async def list_records(
    ...
    items, total = await service.list_paginated(page, page_size, resolution_status)
...
class RecordChatPayload(BaseModel):
    message: str


@router.post("/{record_id}/chat", response_model=APIResponse[RecordResponse])
async def record_chat(
    record_id: uuid.UUID,
    payload: RecordChatPayload,
    current_user: User = Depends(get_current_user),
    service: KnowledgeService = Depends(get_knowledge_service),
    db: AsyncSession = Depends(get_db_session),
) -> APIResponse[RecordResponse]:
    record = await service.record_chat(record_id, payload.message)
    await db.commit()
    return APIResponse.ok(RecordResponse.model_validate(record))


@router.post("/{record_id}/documents", response_model=APIResponse[RecordResponse])
async def upload_record_document(
    ...
    record = await service.upload_and_summarize_document(
        record_id=record_id,
        filename=file.filename or "belge.txt",
...
@router.delete("/{record_id}/documents/{doc_id}", response_model=APIResponse[RecordResponse])
async def delete_record_document(
    record_id: uuid.UUID,
    doc_id: str,
    current_user: User = Depends(get_current_user),
    service: KnowledgeService = Depends(get_knowledge_service),
    ...
    record = await service.delete_document(record_id, doc_id)
    await db.commit()
    return APIResponse.ok(RecordResponse.model_validate(record))


@router.get("/{record_id}/a3-preview", response_model=APIResponse[dict])
async def get_a3_preview(
    record_id: uuid.UUID,
    current_user: User = Depends(get_current_user),
    service: KnowledgeService = Depends(get_knowledge_service),
) -> APIResponse[dict]:
    a3_preview = await service.generate_a3_preview(record_id)
    return APIResponse.ok(a3_preview)


@router.post("/{record_id}/close", response_model=APIResponse[RecordResponse])
async def close_record(
    ...
    record = await service.close_record(record_id, current_user.id)
    await db.commit()
    return APIResponse.ok(RecordResponse.model_validate(record))
