...
class RAGSearchService:
    ...
    async def search(
        ...
        cache_key = self._cache_key(query, methodology, industry, department, assignee_name, start_date, end_date)
        ...
        try:
            ...
            logger.warning(f"RAG Qdrant/Embedding search error: {exc}")
            payloads = []

        if self._cache is not None and payloads:
