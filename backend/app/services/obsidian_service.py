...
class ObsidianService:
    ...
    async def export_record(self, record_id: uuid.UUID) -> Optional[Path]:
        ...
        assignee_val = meta.get("assignee_name") or "Atanmamış"
        tracker_val = meta.get("tracker_name") or "Atanmamış"
        closed_date_val = record.resolution_date.isoformat() if record.resolution_date else ""

        chat_text = ""
        resolution_chat = meta.get("resolution_chat_history", [])
        if resolution_chat:
            for msg in resolution_chat:
                role = "Kullanıcı" if msg["role"] == "user" else "AI Çözüm Ajanı"
                chat_text += f"**{role}:** {msg['content']}\n\n"
        else:
            chat_text = "*Bu problem için henüz çözüm sohbeti gerçekleştirilmedi.*"
...
## 👥 Atamalar & Tarihler
- **Sorumlu (Assignee):** {assignee_val}
- **Denetçi (Tracker):** {tracker_val}
- **Açılış Tarihi:** {record.created_at.strftime('%d.%m.%Y %H:%M') if record.created_at else 'Bilinmiyor'}
- **Kapanış Tarihi:** {record.resolution_date.strftime('%d.%m.%Y %H:%M') if record.resolution_date else 'Açık'}
...
"""
    ...
    async def export_record_to_obsidian(self, record: ProblemRecordORM) -> Optional[Path]:
        return await self.export_record(record.id)
