class GeminiClient:
    ...
    def embed_text(self, text: str) -> list[float]:
        ...
        self._ensure_configured()
        model_name = self._embedding_model_name if self._embedding_model_name.startswith("models/") else f"models/{self._embedding_model_name}"
        try:
            result = genai.embed_content(model=model_name, content=text)
            return list(result["embedding"])
        except Exception:
            # Fallback for API endpoint variations
            result = genai.embed_content(model=self._embedding_model_name, content=text)
            return list(result["embedding"])
