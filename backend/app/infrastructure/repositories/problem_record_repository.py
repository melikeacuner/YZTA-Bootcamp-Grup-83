...
class ProblemRecordRepository:
    ...
    async def list_paginated(
        ...
        query = select(ProblemRecordORM)
        count_query = select(ProblemRecordORM)
        if resolution_status is not None:
            query = query.where(ProblemRecordORM.resolution_status == resolution_status)
            count_query = count_query.where(ProblemRecordORM.resolution_status == resolution_status)

        items_result = await self._session.execute(
            query.order_by(ProblemRecordORM.created_at.desc()).offset(offset).limit(page_size)
        )
