...
def build_filter(
    *,
    ...
    if assignee_name:
        conditions.append(qmodels.FieldCondition(key="assignee_name", match=qmodels.MatchValue(value=assignee_name)))
    if start_date:
        conditions.append(qmodels.FieldCondition(key="resolution_date", range=qmodels.Range(gte=start_date)))
    if end_date:
        conditions.append(qmodels.FieldCondition(key="resolution_date", range=qmodels.Range(lte=end_date)))
